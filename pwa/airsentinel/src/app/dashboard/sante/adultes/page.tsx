  ...
  useEffect(() => {
    ...
    const fetchHealthData = async () => {
      ...
      try {
        ...
        const adulteRec = data.find(r => r.profil.toLowerCase().includes("adulte"));
        setRecommendations(adulteRec ? [adulteRec] : []);
      } catch (err) {
      ...
      <main className="p-6 pb-24 max-w-5xl mx-auto animate-in fade-in duration-700">
        ...
        <header className="mb-10">
          <h1 className="text-4xl font-black text-[#e0f2fe] mb-2 tracking-tight flex items-center gap-3">
            {t('health_title_1')} <span className="text-[#ef4444]">{t('health_prof_adults')}</span> <span className="text-3xl">🧑</span>
          </h1>
          <p className="text-[#e0f2fe]/50 text-sm font-medium italic">{t('health_desc_adults')}</p>
        </header>
